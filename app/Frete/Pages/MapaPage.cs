...
namespace EmagineFrete.Pages
{
    public class MapaPage : ContentPage
    {
        ...
        private PontoTransporte _ponto;
        private List<PontoTransporte> _Pai;
        private ListView _ListaLocais;

        public MapaPage(string pinLabel, ref PontoTransporte ponto, ref List<PontoTransporte> pai, ref ListView lista)
        {
            _ponto = ponto;
            _Pai = pai;
            _ListaLocais = lista;
        ...
        private async Task iniciaBuscaAsync()
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    HttpResponseMessage response = await client.GetAsync(Utils.HttpUtils.montaLinkGet(
        ...
        private void inicializarComponente() {
            ...
            _Mapa.aoClicar += async (sender, e) => {
               await pegarManualAsync(e.Position);
            };
            ...
            _EntregarButton.Pressed += (sender, e) => {
                if(_Mapa.Pins.Count() > 0){
                    ...
                    if(_ponto.Tipo == TipoPontoTransporte.Add){
                        _ponto.Tipo = TipoPontoTransporte.Trecho;
                        _Pai.Add(new PontoTransporte() { Text = "", Tipo = TipoPontoTransporte.Add });
                    }
            ...
            _LocaisLista.ItemTemplate = new DataTemplate(typeof(DefaultCell));
