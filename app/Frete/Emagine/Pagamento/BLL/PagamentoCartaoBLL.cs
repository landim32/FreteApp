...
{
    public class PagamentoCartaoBLL : RestAPIBase
    {
        public async Task<List<PagamentoCartaoInfo>> listar(int idUsuario)
        {
            string url = GlobalUtils.URLAplicacao + "/api/cartao/listar/" + idUsuario.ToString();
            return await queryGet<List<PagamentoCartaoInfo>>(url);
        }
